...
class AttendanceReportModel extends Model
{
    ...
    protected $allowedFields = ['school_id', 'class', 'date', 'absent_count', 'present_count', 'leave_count'];

    /**
     * @throws \ReflectionException
     */
    public function prepareAttendanceReport($from_date, $to_date)
    {
        ...
        for ($date = $from_date; $date <= $to_date; $date = $date->modify('+1 day')) {
            foreach ($school_list as $school) {
                ...
                if (!empty($class_wise_student_count)) {
                    ...
                    $attendance_status_count = $attendance_model->prepareDailyAttendanceReport(1105004, $date);

                    if (!empty($attendance_status_count)) {
                        foreach ($attendance_status_count as $row) {
                            $attendance_report[$row['school_id']][$row['class']] = [
                                "date" => $date->format("Y-m-d"),
                                "school_id" => $attendance_report[$row['school_id']],
                                "class" => $row['class'],
