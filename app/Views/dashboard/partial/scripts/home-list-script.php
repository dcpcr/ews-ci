...
} elseif (isset($response['total_student_list'])) {
...
if (!empty($data) & empty($table_data)) {
    foreach ($data as $case) {
        $table_data [] = [
            "serial_no" => ++$counter,
...
<script>
    const homePageListData = <?=json_encode($table_data)?>;
</script>

<script src='/assets-adminlte/ews-js/home-page-list.js'></script>