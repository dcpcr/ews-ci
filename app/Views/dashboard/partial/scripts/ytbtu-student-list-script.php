...
if (!empty($response['yet_to_be_brought_back_to_school_by_call'])) {
    $data = $response['yet_to_be_brought_back_to_school_by_call'];
} elseif (!empty($response['yet_to_be_brought_back_to_school_by_sms'])) {
    $data = $response['yet_to_be_brought_back_to_school_by_sms'];
}
...
<script>
    const homePageDetectedStudentListData = <?=json_encode($table_data)?>;
</script>

<script src='/assets-adminlte/ews-js/detected-student-list-script.js'></script>